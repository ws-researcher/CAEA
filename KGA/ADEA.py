...
class ADEA(keras.Model):
    ...
    def __init__(self, arg, node_size, rel_size, attr_size, index_matrix=None, all_matix=None, attr_matrix=None, *args, **kwargs):
        ...
        self.concept_emb = concatenate([concept_rel, concept_attr])

    def call(self, AVH=None, training=None, mask=None):
        # -----------------------------------------------------------------------------------
        # AVH = AVH[:,:,0:1]
        ...
        # ------------------------------------------------------------------------------------
        ent_emb = self.entLayer(self.ent_emb, rel_emb=self.rel_emb, attr_emb=self.attr_emb, index_matrix=self.index_matrix, all_matix=self.all_matix, attr_matrix=self.attr_matrix, concept_attr=concept_attr, concept_rel=concept_rel)
    ...
    def call(self, inputs, **kwargs):
        ...
        for l in range(self.args.num_layers):
            ...
            for head in range(self.args.head):
                ...
                concept_attn = tf.squeeze(dot(concatenate([w_self_concept_feature, w_neighs_concept_feature]), concept_attn_kernel), axis=-1)
